...
char* hex(int num)
{
	...
	while(num)
	{
		int tmp=num%16;
		if(tmp>9) {
			*pt=tmp+55;//把大于9的数转化成A,B,C... 
		}else{
			*pt=tmp+48;//把小于10的数转化成0,1,2,3... 
		} 
		num=num/16;
		pt++;
	}
 ...
 * 参数 IV: 初始向量串。
 */
void aes_CBC(char *p, int plen, char *key, char *IV)
{
    ...
    int IVArray[4][4];
    int k,i;
    char group_result[17];
    int group_result_Array[4][4];
    ...
    addRoundTowArray(pArray,IVArray);
    extendKey(key);//扩展密钥
    printW();
    for(k = 0; k < plen; k += 16)
    {
        convertToIntArray(p+k, pArray);
        if(k!=0)//与上一组结果或IV进行异或
        {
            convertToIntArray(IV, IVArray);
            addRoundTowArray(pArray,IVArray);
        }else{
            convertToIntArray(IV, IVArray);
            addRoundTowArray(pArray,group_result_Array);
        ...
        convertArrayToStr(pArray, group_result);

        strcpy(p+k,group_result);
...
void aesStrToFile(char *key)
{
    ...
    printf("轮密钥..................\n");
    aes_CBC(p, plen, key,IV);//AES加密,CBC模式
    //aes(p,plen,key);
    printf("进行AES_CBC加密..................\n");
